...
contract Staking is IStaking, InjectorContextHolder {
    ...
    function _delegateTo(address fromDelegator, address toValidator, uint256 amount) internal {
        ...
        require(amount >= _chainConfigContract.getMinStakingAmount() && amount != 0, "too low");
    ...
    function _undelegateFrom(address toDelegator, address fromValidator, uint256 amount) internal {
        ...
        if(!_validatorsUnlock[fromValidator]&& msg.sender==validator.ownerAddress){revert('unlock');}
    ...
    function _claimDelegatorRewardsAndPendingUndelegates(address validator, address delegator, uint64 beforeEpochExclude) internal {
    ...
    function _calcDelegatorRewardsAndPendingUndelegates(address validator, address delegator, uint64 beforeEpoch) internal view returns (uint256) {
        ...
        while (delegation.delegateGap < delegation.delegateQueue.length) {
            ...
            for (; delegateOp.epoch < beforeEpoch && (voteChangedAtEpoch == 0 || delegateOp.epoch < voteChangedAtEpoch); delegateOp.epoch++) {
                ValidatorSnapshot memory validatorSnapshot = _validatorSnapshots[validator][delegateOp.epoch];
                availableFunds +=_sysReward(validator,delegator,delegateOp.amount);
                if (validatorSnapshot.totalDelegated == 0) {
                    continue;
                }
                (uint256 delegatorFee, /*uint256 ownerFee*/, /*uint256 systemFee*/) = _calcValidatorSnapshotEpochPayout(validatorSnapshot);
    ...
    function registerValidator(address validatorAddress, uint16 commissionRate) payable external override {
        ...
        require(initialStake >= _chainConfigContract.getMinValidatorStakeAmount(), "too low");
    ...
    function _addValidator(address validatorAddress, address validatorOwner, ValidatorStatus status, uint16 commissionRate, uint256 initialStake, uint64 sinceEpoch) internal {
        ...
        require(commissionRate >= COMMISSION_RATE_MIN_VALUE && commissionRate <= COMMISSION_RATE_MAX_VALUE, "bad rate");
        // init validator default params
        Validator memory validator = _validatorsMap[validatorAddress];
        require(_validatorsMap[validatorAddress].status == ValidatorStatus.NotFound, "val is use");
    ...
    function unlockValidator(address validator) external onlyFromGovernance virtual override {
    ...
    function changeValidatorOwner(address validatorAddress, address newOwner) external override {
        ...
        ValidatorDelegation storage delegation = _validatorDelegations[validatorAddress][newOwner];
        require(delegation.delegateQueue.length == 0, "is use");
        delete _validatorOwners[validator.ownerAddress];
        _validatorDelegations[validatorAddress][newOwner]=_validatorDelegations[validatorAddress][validator.ownerAddress];
    ...
    function _sysReward(address validators,address delegator,uint112 money) internal view  returns (uint256) {

        Validator memory validator = _validatorsMap[validators];

        if(_nowReward < _chainConfigContract.getMaxSystemRewards()&&validator.status== ValidatorStatus.Active){

        if(validator.ownerAddress == delegator){
             return uint256(money*10/36500)*BALANCE_COMPACT_PRECISION;
        }
